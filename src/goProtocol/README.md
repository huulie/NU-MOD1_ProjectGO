...
2. De eerste versie (versie 1.0) van het protocol moet altijd ondersteund blijven.
...
de staat van elk veld. De staat van elk veld wordt vertegenwoord door de hiervoor gereserveerde symbolen in `PROTOCOL.`:
- `U` = Unoccupied (leeg)
- `W` = White
- `B` = Black
...
document en op GitHub). Als de requestedVersion aanwezig is, zal de server deze gebruiken. Anders moet 
de altijd ondersteunde versie van het protocol worden gebruikt.
...
3. Bij valide move (of passen), stuurt server:  
...
Client wacht op verder bericht van de server.
...
`scoreBlack` en `scoreWhite` zijn textuele representaties van een double;   
